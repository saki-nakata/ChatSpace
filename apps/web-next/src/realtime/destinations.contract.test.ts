 ...
 *
 * <p>`apps/api-java`の`StompDestinationsExporter`(フェーズ8で新設)が書き出す
 * `build/generated/stomp-destinations.json`と、このリポジトリの`destinations.ts`(手動同期)を突き合わせ、
 * ドリフト(バックエンド側の宛先変更にフロントエンドが追随できていない状態)を検出する。
 *
 * <p>JSONファイルは`cd apps/api-java && ./gradlew exportStompDestinations`で生成する(コミット対象外の
 * ビルド成果物)。CI(`frontend-next-ci.yml`)ではこのテストの前に同タスクを実行する。
 */
describe("STOMP宛先の契約テスト(Java/TS突き合わせ)", () => {
  it("apps/api-javaが出力するJSONと destinations.ts の定数が一致する", () => {
    const jsonPath = fileURLToPath(
      new URL("../../../api-java/build/generated/stomp-destinations.json", import.meta.url),
    );
    ...
    try {
      ...
      throw new Error(
        "stomp-destinations.jsonが見つかりません。先に" +
          " `cd apps/api-java && ./gradlew exportStompDestinations` を実行してください。",
      );
    }
