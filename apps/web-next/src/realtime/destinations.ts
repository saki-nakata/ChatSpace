/**
 * STOMP宛先名(リアルタイム通信機能定義書§4)。`apps/api-java`の`StompDestinations.java`を正とし、手動で同期する。
 *
 * <p>ドリフト防止のため、Vitestの契約テスト(`destinations.contract.test.ts`)が
 * `apps/api-java/build/generated/stomp-destinations.json`(`./gradlew exportStompDestinations`で生成)と
