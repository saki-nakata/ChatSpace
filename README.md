...
- 旧実装(`apps/api`, `apps/web`)は、新実装が機能同等性チェックリストを満たすまで並行して残す(削除しない)。現時点(フェーズ5完了)では新バックエンド `apps/api-java` に認証・ワークスペース/チャンネル/DM CRUD・メッセージング(編集/削除/スレッド/リアクション)・STOMPによるリアルタイム通信・メンション/通知(スコープ再チェック含む)まで実装済み。検索・添付ファイル・OpenAPI生成・フロントエンド(`apps/web-next`)は未着手
