...
- 旧実装(`apps/api`, `apps/web`)は、新実装が機能同等性チェックリストを満たすまで並行して残す(削除しない)。現時点(フェーズ0完了)では新バックエンド `apps/api-java` はリポジトリ雛形(Gradle/Spotless/ArchUnit/Flywayマイグレーション)のみで、認証・API等の機能はまだ実装されていない
