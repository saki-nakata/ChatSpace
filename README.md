...
**公開デモ**: [chatspace-ydxk.onrender.com](https://chatspace-ydxk.onrender.com)(誰でも新規登録可能。無料インスタンスのため、しばらくアクセスが無いとスリープし初回アクセス時に起動まで1分ほどかかる場合がある。添付ファイルのウイルススキャンは未実装のため、本物の個人情報は登録しないこと。濫用対策として新規登録・ログインにIP単位のレート制限があり、同一ネットワークから短時間に多数の登録を行うと一時的に `429` が返る)

> **⚠️ 公開デモは 2026-09-22 で終了します。** 本番のデータベースが Render の無料 PostgreSQL(作成から30日で失効)であるため、この日以降デモは動作しなくなります。有料化・他サービスへの移行は行わない方針です(本リポジトリは学習目的で、デモの常時稼働を目的としていないため)。この日以降にアプリを試したい場合は、下記「セットアップ」の手順でローカル起動してください。移行を検討する場合の判断材料は [docs/インフラ構成書.md §4.2.1](docs/インフラ構成書.md) を参照。
