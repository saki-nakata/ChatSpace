...
| [フェーズ12](phase12.md) | 仕上げ(レート制限、SameSite/CSRF再確認、ログ、情報漏洩監査) | ✅ 完了(SameSite/CSRFは本番ドメイン未定のため判断基準を文書化して保留) |
...
| [フェーズ15](phase15.md) | (任意)パフォーマンステスト | 未着手・任意 |

## レビュー対応

フェーズ1〜8完了時点のコード全体を対象にレビューを実施し、指摘事項全項目を修正済み。詳細は [review-fixes-2026-08-16.md](review-fixes-2026-08-16.md) を参照(プライベートチャンネルのSTOMP情報漏洩・`reactedByMe`の配信不具合・トランザクション境界・Service層認可の多層防御・例外処理の網羅性・その他軽微な指摘)。

## 実装方針の要点

- **認可を最大リスクと位置づけ、各フェーズで対応する認可・受け入れテストを先に(失敗する状態で)書き、実装と同時に通す**(テスト設計書.md参照)
- 単一インスタンス前提で完結させる。水平スケール対応は任意の学習発展フェーズとして検討したが、実施しないことが確定した(検討経緯は`docs/インフラ構成書.md`§4.3参照)。フェーズ13は添付ファイルのオブジェクトストレージ化(Cloudflare R2)、フェーズ14はRenderデプロイ本体に差し替えている
